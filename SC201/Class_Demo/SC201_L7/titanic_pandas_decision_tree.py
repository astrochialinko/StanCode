...
def main():
    ...
    # Construct Tree
    h = tree.DecisionTreeClassifier(max_depth=5) # max_leaf_nodes=10, min_samples_leaf=2
    classifier = h.fit(x_train, y)
    print(classifier.score(x_train, y))
    tree.export_graphviz(classifier, out_file='tree.dot', feature_names=feature_names)

    x_test = data_preprocess(TEST_FILE, mode = 'test')
    x_test = x_test[feature_names]
    predictions = classifier.predict(x_test)
    out_file(predictions, 'decision_tree_tatanic.csv')
    # decistion tree, max_depth=5
