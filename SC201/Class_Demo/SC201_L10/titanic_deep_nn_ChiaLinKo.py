...
def main():
    ...
    Ai['A0'] = X
    for i in range(1, L):
        Ki['K'+str(i)] = np.dot(weights['W'+str(i)].T, Ai['A'+str(i-1)]) + biases['B'+str(i)]
        Ai['A'+str(i)] = np.maximum(Ki['K'+str(i)], 0)
    scores = np.dot(weights['W'+str(L)].T, Ai['A'+str(L-1)]) + biases['B'+str(L)]
...
def neural_network(X, Y):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i in range(L, 0, -1):
            if i == L:
                dKi['K'+str(i)] = (1/m)*np.sum(H-Y, axis=0, keepdims=True)
            else:
                dAi['A'+str(i)] = np.dot(weights['W'+str(i+1)], dKi['K'+str(i+1)])
                dKi['K'+str(i)] = dAi['A'+str(i)] * np.where(Ki['K'+str(i)]>0, 1, 0)    
            dWi['W'+str(i)] = np.dot(Ai['A'+str(i-1)], dKi['K'+str(i)].T)
            dBi['B'+str(i)] = np.sum(dKi['K'+str(i)], axis=1, keepdims=True)

        # Updates all the weights and biases
        for i in range(L, 0, -1):
            weights['W'+str(i)] -= ALPHA*dWi['W'+str(i)]
            biases['B'+str(i)] -= ALPHA*dBi['B'+str(i)]
